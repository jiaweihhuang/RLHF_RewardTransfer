...
run_iteration() {
    ...
    if [ ! -f $reward_label_output ]; then
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --main_process_port 21500 annotate_data/Annotation.py --dataset_name_or_path $json_output --output_dir $reward_label_output --prev_win_rate_output_dir $prev_win_rate_output_dir --win_rate_output_dir $win_rate_output_dir --K 8 --Source_RMs_Names rougeLsum+BERTScore --Source_LLMs_Names google-t5/t5-base+google-t5/t5-large --judge_model_tokenizer meta-llama/Meta-Llama-3-8B-Instruct --judge_model_path meta-llama/Meta-Llama-3-8B-Instruct  --replace_index 0 --set_eval True --N 32 --seed $[$seed + 123] --replace_num 4
    else
...
do
    ...
    base_path="${parent_path}/DPO_Transfer_K8_N32_Rp4_TS10K_Ep3"
    mkdir -p $base_path
