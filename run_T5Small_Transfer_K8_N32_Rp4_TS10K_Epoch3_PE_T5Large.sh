...
run_iteration() {
    ...
    if [ ! -f $json_output ]; then
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --main_process_port 29506 generation/gen_hf_local.py --eos_ids 128009 --tokenizer $pretrained_model_path --pretrained_model $pretrained_model_path --dataset_name_or_path $json_input --dataset_key document --output_dir $json_output --K 8 --N 32 --temperature 1.0 --batch_size 8 --train_data_size 10000 --dtype bfloat16 --set_eval True --seed $seed
    else
    ...
    if [ ! -f $reward_label_output ]; then
        ...
        # first, label the first K responses by human preferences
        # second, label the rest N responses by imperfect reward model (to approximate BoN)
        CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --main_process_port 29506 annotate_data/Annotation.py --dataset_name_or_path $json_output --output_dir $reward_label_output --prev_win_rate_output_dir $prev_win_rate_output_dir --win_rate_output_dir $win_rate_output_dir --K 8 --Source_RMs_Names rougeLsum+BERTScore --Source_LLMs_Names google-t5/t5-base+google-t5/t5-large --judge_model_tokenizer meta-llama/Meta-Llama-3-8B-Instruct --judge_model_path meta-llama/Meta-Llama-3-8B-Instruct  --replace_index 0 --set_eval True --N 32 --seed $[$seed + 123] --replace_num 4 --pure_exploit LLM_google-t5/t5-large
    else
...
do
    ...
    base_path="${parent_path}/DPO_Transfer_K8_N32_Rp4_TS10K_Ep3_PE_T5Large"
    mkdir -p $base_path
